...
def pay_for_5_bundle(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            "description": "471.70MB Bundle",
            "callbackUrl": 'https://webhook.site/d53f5c53-eaba-4139-ad27-fb05b0a7be7f',
            "returnUrl": f'https://bestpay-app-id6nm.ondigitalocean.app/send_5_mtn_bundle/{client_ref}/{phone_number}',
            "cancellationUrl": "https://www.google.com",
            "merchantAccountNumber": "2017101",
            "clientReference": client_ref
            })
    ...
    for request in webhook_response.json()['data']:
        try:
            try:
                ...
                return redirect(f"https://bestpay-app-id6nm.ondigitalocean.app/send_5_mtn_bundle/{client_ref}/{phone_number}")
            status = content["Status"]
            ref = content["Data"]["ClientReference"]
        except KeyError:
            return redirect("failed")
    ...
    for request in webhook_response.json()['data']:
        try:
            try:
                ...
                return redirect(f"https://bestpay-app-id6nm.ondigitalocean.app/send_400_mtn_bundle/{client_ref}/{phone_number}")
            status = content["Status"]
            ref = content["Data"]["ClientReference"]
        except KeyError:
            return redirect("failed")
