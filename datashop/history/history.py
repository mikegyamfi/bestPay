...
def airtime_transactions(request, username):
    all_airtime_transactions = models.AirtimeTransaction.objects.filter(username=username).first()
...
def voda_bundle_transactions(request, username):
    all_voda_bundle_transactions = models.VodafoneBundleTransaction.objects.filter(username=username).first()
...
def mtn_bundle_transactions(request, username):
    all_mtn_bundle_transactions = models.MTNBundleTransaction.objects.filter(username=username).first()
    context = {'txns': all_mtn_bundle_transactions, 'heading': "MTN Bundle Transactions"}
    return render(request, "bundle_table.html", context=context)


def tigo_bundle_transactions(request, username):
    all_tigo_bundle_transactions = models.AirtelTigoBundleTransaction.objects.filter(username=username).first()
...
def other_mtn_bundle_transactions(request, username):
    all_other_mtn_bundle_transactions = models.AirtimeTransaction.objects.filter(username=username).first()
...
def sika_kokoo_bundle_transactions(request, username):
    all_sika_kokoo_transactions = models.SikaKokooBundleTransaction.objects.filter(username=username).first()
...
def ishare_bundle_transactions(request, username):
    all_ishare_transactions = models.SikaKokooBundleTransaction.objects.filter(username=username).first()
...
def tv_transactions(request, username):
    all_tv_transactions = models.SikaKokooBundleTransaction.objects.filter(username=username).first()
