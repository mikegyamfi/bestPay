...
def pay_for_gotv(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            "returnUrl": f'https://bestpay-app-id6nm.ondigitalocean.app/send_gotv_amount/{client_ref}/{account_number}/{amount}',
            ...
            return render(request, 'store/layouts/gotv.html', context={'form': form})
    else:
        form = TVForm()
    return render(request, "store/layouts/gotv.html", {'form': form})
    ...
    for request in webhook_response.json()['data']:
        try:
            try:
                ...
                return redirect(f"https://bestpay-app-id6nm.ondigitalocean.app/send_gotv_amount/{client_ref}/{account_number}/{amount}")
            status = content["Status"]
            ref = content["Data"]["ClientReference"]
        except KeyError:
            return redirect("failed")
        if ref == client_ref and status == "Success":
            url = "https://cs.hubtel.com/commissionservices/2016884/e6ceac7f3880435cb30b048e9617eb41"
